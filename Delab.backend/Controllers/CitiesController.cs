...
public class CitiesController : ControllerBase
{
    ...
    {
        var ListItems = await _context.Cities.OrderBy(Orden => Orden.Name).ToListAsync();   //Pedir la lista de paises ordenado por Nombre
        return Ok(ListItems);
    }

    [HttpGet("{id}")]   // Listar solo los registros del ID osea un solo registro
    public async Task<ActionResult<IEnumerable<City>>> GetListAsync(int id)
    {
        try
        {
            var ItemModelo = await _context.Cities.FindAsync(id);   // Opcion 1
    ...
    public async Task<IActionResult> PostAsync([FromBody] City modelo)  // Modelo es un sobrenombre que se se asigna al modelo que da acceso a la BD
    {
        try
        {
            _context.Cities.Add(modelo); // Segurar el nombre de la tabla
    ...
    public async Task<ActionResult<City>> PutAsync(City modelo)
    {
        try
        {
            ...
            UpdateModelo!.StateId = modelo.StateId;
